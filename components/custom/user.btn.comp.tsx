...
export default function UserBtn({ session }: { session: Session | null }) {
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent className="w-56 rounded-2xl mr-5">
        <DropdownMenuLabel className="flex flex-row gap-3 cursor-pointer">
          ...
          <div className="flex flex-col">
            <span className="text-md">{session?.user?.name || "UserName"}</span>
          </div>
        </DropdownMenuLabel>
        <DropdownMenuSeparator />
        <DropdownMenuGroup>
